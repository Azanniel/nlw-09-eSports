...
const daysWeek = ['Domingo', 'Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta']
      ...
      <Dialog.Content className='fixed bg-[#2A2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] shadow-lg shadow-black/25'>
        ...
        <form onSubmit={handleCreateAd} className="mt-8 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input id="name" name="name" type="text" placeholder="Seleciona o game que deseja jogar" />
          </div>
                ...
                {daysWeek.map((day, index) => {
                  return (
                    <ToggleGroup.Item
                      ...
                      value={String(index)}
                      title={day}
                      className={`w-8 h-8 rounded ${weekDays.includes(String(index)) ? 'bg-violet-500' : 'bg-zinc-900'}`}
                    >
                      {day.charAt(0)}
                    </ToggleGroup.Item>
                  )
